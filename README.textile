...
If a subset of databases is required then specify each one using the <code>-databases</code> flag when initalising the pipeline 
...
h3. Using files downloaded via another mechanism (assumes files and dirs are in work_directory)
...
h3. Using rsync for the transport protocol
