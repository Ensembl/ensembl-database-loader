...
sub fetch_input {
  my ($self) = @_;
  throw 'No work_directory has been given'
    unless $self->param('work_directory');
...
sub download {
  ...
  if ( $self->param('rsync') ) {
    ...
    print STDERR
      "Switching to FTP as the submitted host does not support rsync\n"
      if $self->debug();
  }
  $self->_ftp_download();
  return;
...
sub _ftp_download {
  ...
  foreach my $file ( @{ $ls->{files} } ) {
    $ftp->get($file) or
      throw "Cannot get the file $file from FTP " . $ftp->message();
  }
  $self->disconnect_ftp();
  return;
}
...
sub _rsync_download {
  ...
  if ( $rc != 0 ) {
    $self->throw(
"Encountered a problem whilst downloading files for ${database}. Rsync command was '$cmd'"
    );
  }
  $self->cwd_local_dir($database);
  return;
...
sub checksum {
  ...
  foreach my $file ( sort keys %{$given_checksums} ) {
    ...
    if ( $actual ne $expected ) {
      ...
      warning(sprintf( q{'%s' did not checksum. Expected %s but got %s},
  ...
  if ( -d $target ) {
    if ($remove_if_exists) {
      if ( $self->input_job()->retry_count() == 0 ) {
        throw
"Cannot create the directory '${target}' as it already exists. Remove and rerun";
      }
      rmtree($target);
    }
  }
...
sub _parse_checksums_file {
  ...
  my $fh;
  if ( -f 'CHECKSUMS.gz' ) {
    $fh = IO::Uncompress::Gunzip->new('CHECKSUMS.gz') or
      throw "gunzip failed: $GunzipError\n";
  }
  elsif ( -f 'CHECKSUMS' ) {
